...
import {TicketNft} from "../src/EventNft.sol";
...
contract EventTicketingTest is Test {
    ...
    // function testFuzz_CreateTicket(uint256 price, uint256 supply, uint256 nftSupply) public {
    //     price = bound(price, 0.001 ether, 10 ether);
    //     supply = bound(supply, 1, 10000);
    //     nftSupply = bound(nftSupply, 1, 10000);
        
    //     vm.prank(owner);
    //     eventTicketing.createTicket(price, supply, nftSupply);
        
    //     (, , uint256 storedPrice, uint256 storedSupply, , uint256 storedNftSupply, ) = eventTicketing.tickets(0);
    //     assertEq(storedPrice, price);
    //     assertEq(storedSupply, supply);
    //     assertEq(storedNftSupply, nftSupply);
    // }
    
    // function testFuzz_BuyTicket(uint256 amount, uint256 payment) public {
    //     createSampleTicket();
        
    //     amount = bound(amount, 1, TICKET_SUPPLY);
    //     uint256 requiredPayment = TICKET_PRICE * amount;
    //     payment = bound(payment, requiredPayment, requiredPayment + 10 ether);
    ...
    //     vm.prank(user1);
    //     eventTicketing.buyTicket{value: payment}(0, amount);
